...
def stitch_replacement(dict_lib, target_image, output_image, pixel_width, pixel_height, real_color_diff):
    ...
    while start_x < target_width and start_y < target_height:
        ...
        closest, skew = get_closest_lib_img(target_section, dict_lib, real_color_diff)
        img_closest = skew_image(closest, skew)
        if cur_row == None:
            cur_row = img_closest
        else:
            cur_row = np.concatenate([cur_row, img_closest], axis=1)
...
def skew_image(img, skew):
    cv_image = cv2.imread(img)
    for row in xrange(0,cv_image.shape[0]):
        for col in xrange(0,cv_image.shape[1]):
            cv_image[row,col][0] += skew[0]
            cv_image[row, col][1] += skew[1]
            cv_image[row, col][2] += skew[2]
    return cv_image

#move color_1 towards color_2
def get_skew(color_1, color_2):
    r_skew = int(math.floor((color_2[0]-color_1[0])/2))
    g_skew = int(math.floor((color_2[1]-color_1[1])/2))
    b_skew = int(math.floor((color_2[2]-color_1[2])/2))
...
def simple_color_diff(color_1, color_2):
    ...
    return r_diff + g_diff + b_diff, get_skew(color_1, color_2)
...
def color_diff(color_1, color_2):
    ...
    return delta_e, get_skew(color_1, color_2)
...
def get_closest_lib_img(target_section, dict_lib, real_color_diff):
    ...
    for key, value in dict_lib.iteritems():
        dist, rgb_skew = dist_rgb(value, (r,g,b), real_color_diff)
        if winning_img == None or dist < winning_dist:
            winning_dist = dist
            winning_img = key
            winning_skew = rgb_skew
    return winning_img, winning_skew
